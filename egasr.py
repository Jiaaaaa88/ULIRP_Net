...
class CoreModule(nn.Module):
    def __init__(self, features, M, G, r, stride=1, L=32):
        ...
        self.recon_trunk = common.ResidualBlock_noBN(nf=features, at='prelu') #创建残差块
        ...
        self.fc = nn.Linear(features, d) #降维
...
class EGRAB(nn.Module):
    ...
    def forward(self, x):
        res = self.body(x)
        #print('res',res.shape)  #res torch.Size([32, 32, 60, 60])
        res = F.interpolate(res, size=(96, 96), mode='bilinear', align_corners=False)
        x = F.interpolate(x, size=(96, 96), mode='bilinear', align_corners=False)
        #print('x',x.shape) #torch.Size([32, 32, 58, 58])
        res += x
        return res
...
class EGASR(nn.Module):
    def __init__(self, conv=common.default_conv):
        ...
        n_feats = 32 #number of feature maps
        kernel_size = 3
        ...
        # define head module
        modules_head = [conv(n_colors, n_feats, kernel_size)] #channel,batchsize,hw
...
if __name__ == '__main__':
    ...
    input11 = torch.randn( 3, 3,96, 96)
    #input11 = F.interpolate(input11, size=(64, 64), mode='bilinear', align_corners=False)
    #input_permuted = input11.permute(1, 0, 2, 3)
